...
def build_all_versions(repo_path, docs_dir="docs", builder="html", push=False, check_dirty=True, force_theme=True, ignore_errors=True, require_git_lfs=False):
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        for ref in all_refs:
            ...
            if require_git_lfs:
                logger.info(f"Pulling Git LFS objects for {ref.name}...")
                try:
                    repo.git.lfs('pull')
                except Exception as e:
                    logger.warning(f"Failed to pull LFS objects for {ref.name}: {e}")            
...
def main():
    ...
    # FIX: Updated to '--require-git-lfs' to match the rest of the script
    parser.add_argument("--require-git-lfs", action="store_true", help="Enable Git LFS support (requires git-lfs)")
    args = parser.parse_args()
    
    # FIX: Passed as 'require_git_lfs=args.require_git_lfs'
    build_all_versions(
        Path.cwd(), 
        push=args.push, 
        force_theme=not args.no_force_theme, 
